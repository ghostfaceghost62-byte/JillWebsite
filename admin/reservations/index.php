                    ...
                    <tr>
                        ...
                        <td>
                            <?php if ($pStatus === 'PENDING_APPROVAL'): ?>
                                <span class="badge" style="background:#FEF3C7;color:#92400E;font-weight:700;padding:0.25rem 0.6rem;border-radius:4px;display:inline-block;margin-bottom:0.3rem;">⚡ UNDER REVIEW</span>
                                <br>
                                <button class="btn-gold-inspect" type="button" onclick="openPaymentModal(<?=htmlentities(json_encode($r))?>)">📷 Inspect &amp; Verify Proof</button>
                            <?php elseif ($pStatus === 'PAID'): ?>
                                <span class="badge" style="background:#ECFDF5;color:#065F46;font-weight:700;padding:0.2rem 0.5rem;border-radius:4px;display:inline-block;">✅ PAID</span>
                                <?php if (!empty($r['online_ref_code'])): ?>
                                    <br><small style="font-family:monospace;font-size:0.75rem;color:var(--text-secondary);font-weight:600;">Ref: <?=e($r['online_ref_code'])?></small>
                                <?php endif; ?>
                                <br>
                                <button class="btn-crimson-inspect" type="button" onclick="openPaymentModal(<?=htmlentities(json_encode($r))?>)">📷 Review Proof &amp; Details</button>
                            <?php elseif ($pStatus === 'REJECTED'): ?>
                                <span class="badge" style="background:#FEF2F2;color:#991B1B;font-weight:700;padding:0.2rem 0.5rem;border-radius:4px;display:inline-block;">❌ REJECTED</span>
                                <br>
                                <button class="btn-crimson-inspect" type="button" onclick="openPaymentModal(<?=htmlentities(json_encode($r))?>)">📷 Review Details</button>
                            <?php else: ?>
                                <span class="badge" style="background:#F3F4F6;color:#4B5563;font-weight:700;padding:0.2rem 0.5rem;border-radius:4px;display:inline-block;">UNPAID</span>
                                <br>
                                <button class="btn-crimson-inspect" type="button" onclick="openPaymentModal(<?=htmlentities(json_encode($r))?>)">📷 Inspect Details</button>
                            <?php endif; ?>
                        </td>
                        <td>
